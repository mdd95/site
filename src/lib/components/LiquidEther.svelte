<script lang="ts">
	...
	let webglRef: LiquidEtherWebGL | null = null;
	...
	let resizeRafRef: number | null = null;
...
`;
		...
		class WebGLManager implements LiquidEtherWebGL {
			...
			constructor(props: any) {
				...
				this._onVisibility = () => {
					...
					else if (isVisibleRef) this.start();
				};
			...
			loop() {
				...
				this.render();
				rafRef = requestAnimationFrame(this._loop);
			}
		...
		const applyOptionsFromProps = () => {
			if (!webglRef) return;
			const sim = webglRef.output?.simulation;
			if (!sim) return;
		...
		const io = new IntersectionObserver(
			(entries) => {
				...
				if (!webglRef) return;
				if (isVisible && !document.hidden) webglRef.start();
				else webglRef.pause();
			},
		...
		const ro = new ResizeObserver(() => {
			...
			resizeRafRef = requestAnimationFrame(() => {
				if (!webglRef) return;
				webglRef.resize();
			});
		});
...
<div bind:this={mountRef} {...restProps}></div>
